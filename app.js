...
window.showMeter=id=>{const m=data.find(x=>x.id===id);if(!m)return;detail.innerHTML=`<div class="detail"><h2>${esc(m.meterNo)}</h2><div class="gallery">${(m.photos.length?m.photos:[placeholder(m.meterNo)]).map(p=>`<img src="${p}" alt="${esc(m.meterNo)}">`).join('')}</div><div class="info"><div class="full"><small>Alamat</small><p>${esc(m.address)}</p></div><div><small>Status</small><p>${esc(m.status)}</p></div><div><small>GPS</small><p>${m.lat}, ${m.lng}</p></div>${m.remark?`<div class="full"><small>Remark</small><p>${esc(m.remark)}</p></div>`:''}</div><a class="map" href="${maps(m)}" target="_blank" rel="noopener">📍 Open in Google Maps</a></div>`;dlg.showModal()};
